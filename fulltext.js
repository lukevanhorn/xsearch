...
Fulltext.prototype.add = function(content, document, id, callback) {
	var self = this;
	var id = id || new Date().getTime();
	
	self.onAdd(id, find_keywords(content.replace(REG_TAG, ' '), false), document, callback);
...
Fulltext.prototype.keywords = function(content, alternative, count, max, min) {
	return find_keywords(content, alternative, count, max, min);
};
...
FulltextFile.prototype.add = function(id, keywords, document, callback) {
	...
	fs.writeFile(path.join(self.documents, id + EXTENSION_DOCUMENT), JSON.stringify(document));
